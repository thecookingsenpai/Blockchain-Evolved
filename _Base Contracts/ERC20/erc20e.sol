...
contract ERC20E is IERC20E, protected {
    ...
    // ANCHOR Internal only direct transfer
        ...
        returns (bool success)
    {
        ...
        balances[from] -= amount;
        balances[to] += amount;
        emit Transfer(from, to, amount); //solhint-disable-line indent, no-unused-vars
        return true;
    }
    ...
    function add_liquidity(uint256 token_amount) public payable safe {
        ...
        (native_in_user, token_in_user) = getOwnedTokens(msg.sender);
        // Updating user's share of the pool
        native_in_user += native_amount;
        token_in_user += token_amount;
        _userShareUpdate(msg.sender, native_in_user, token_in_user);
    }
    ...
    /// @dev Updates the user's share of the liquidity pool
    /// @param _user The user to update
    /// @param native_in_user The amount of native tokens in the user's share
    /// @param token_in_user The amount of tokens in the user's share
    function _userShareUpdate(
        address _user,
        uint256 native_in_user,
        uint256 token_in_user
    ) internal {
    ...
    function buy_with_native() public payable safe {
        ...
        bool success = _internalTransfer(address(this), msg.sender, token_amount);
        require(success, "transfer failed");
        token_in_liquidity -= token_amount;
    }
    ...
    function sell_to_native(uint256 amount) public returns (uint256) {
        ...
        bool get = _internalTransfer(msg.sender, address(this), amount);
